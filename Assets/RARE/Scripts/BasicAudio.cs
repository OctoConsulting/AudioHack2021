...
{
    public static BasicAudio instance;
    ...
    void Update()
    {
        ...
        if (isplaying && PlayHeadTouch == false)
    ...
    private string GetNewFileName()
    {
        ...
        return TurretSystem.instance.newShotString;
    }
    public void ListenerStartStop()
    {
        print("ListenerStartStop called: " + (isRecording ? "Stop Recording" : "Start Recording"));
        if (isRecording)
        {
            micRecordButton.SetActive(true);
            playRecordingButton.SetActive(true);
            string newFileName = GetNewFileName();
            RARE.Instance.StopAudioListenerRecording(newFileName, ClipLoaded, popUp);
            recordNum++;
            isRecording = false;
            info.text = "Done.";
    ...
    public void ListenerStop(string newFileName)
    {
        micRecordButton.SetActive(true);
        playRecordingButton.SetActive(true);
        RARE.Instance.StopAudioListenerRecording(CheckFileName(newFileName), ClipLoaded, popUp);
        recordNum++;
        isRecording = false;
        info.text = "Done.";
        audioRecordButton.GetComponentInChildren<Text>().text = "2. Audio Record";
    ...
    public void ListenerStart()
    {
        micRecordButton.SetActive(false);
        playRecordingButton.SetActive(false);
        info.text = "Recording Shot Sound...";
        isRecording = true;
        RARE.Instance.StartAudioListenerRecording();
        audioRecordButton.GetComponentInChildren<Text>().text = "Stop";
    ...
    public void ClipLoaded(AudioClip myClip, string clipName = null)
    {
        ...
        //print("this clip name = " + myClip.name);
    ...
    public void Share()
    { //share audio natively for android and iOS 
#if UNITY_EDITOR
